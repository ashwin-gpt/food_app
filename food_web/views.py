...
def get_live_offers():
    ...
    if not shop:
        return jsonify([
    {
        'imageUrl': url_for('static', filename=row[1]),  # 🔁 Use imageUrl for JS
        'title': f"Limited Time Offer {idx + 1}",
        'subtitle': "Don't miss out on this delicious deal!",
        'badge': "OFFER",
        'ctaText': "Order Now",
        'ctaLink': "#order"
    }
    ...
    try:
        ...
        # Query active banners - check if Banner model has shop_name field
        try:
            # Try filtering by shop_name if the field exists
            ...
            # If shop_name field doesn't exist, get all active offers
            active_offers = Banner.query.filter(
                Banner.start_time <= now,
                Banner.end_time >= now
            ).order_by(Banner.uploaded_at.desc()).all()
        ...
        for i, banner in enumerate(active_offers):
            offers.append({
                ...
                'position': 2,  # Always inject into middle position
                'imageUrl': url_for('static', filename=banner.image_path),
                'title': 'Special Offer',
